...
public:
   ...
   */
   RreqHeader (uint8_t flags = 0, uint8_t reserved = 0, uint8_t hopCount = 0,
              ...
              uint32_t originSeqNo = 0, uint32_t helloInterval = 1);


   void SetHelloInterval (double helloInterval)
   {
	   helloInterval = helloInterval * 1000;
	   m_helloInterval = (uint32_t) helloInterval;
   }

   double GetHelloInterval (void)
   {
	   double hInterval = m_helloInterval;
