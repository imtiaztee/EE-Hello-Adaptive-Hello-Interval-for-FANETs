...
/*
 * proposed and implemented by Imtiaz Mahmud (imtiaz.tee@gmail.com)
 */
void
RoutingProtocol::EnergyCalculator(void)
{

	//if(m_ipv4->GetObject<Node>()->GetId() == 3)
	//{
	/*
		std::ofstream energyOverhead("energy_overhead_D.txt", std::ofstream::out | std::ofstream::app);

		// Always check to see if the file is open and for errors.
		if (energyOverhead.is_open())
		{
		...
		energyOverhead.close();
	*/

		overHeadPerSec.push_back(overHead);
		recOverHeadPerSec.push_back(recOverHead);
		m_energyTimer.Schedule(Seconds (1));
	//}
}
 ...
 */
void
RoutingProtocol::SetHelloInterval(void)
{
	Ptr<GaussMarkovMobilityModel> mobility = m_ipv4->GetObject<Node>()->GetObject<GaussMarkovMobilityModel>();


	double Tx = 150, Um = 40, L = 600, W = 600, H = 150, Vtheta = 0.85, Vc = 0, Vmax = 30, Vmin = 5,  beta = 0.1, Thello = 0;

	networkDensity = (4 * 3.1416 * Um * Tx * Tx * Tx) / (9 * L * W * H);


	Vc = mobility->GetVelocity().GetLength();
	Va = (Vtheta * Vc) + ((1 - Vtheta) * Va);


	Thello = (9 * beta * Vmax * L * W * H) / (4 * 3.1416 * Vmin * Tx * Tx * Um * Va);	//proposed of IEEE Access
	...
	m_helloInterval = Seconds(Thello);

	NS_LOG_UNCOND ("\n\n\n\n\n\n\n\n\n\n\n"<<m_ipv4->GetObject<Node>()->GetId()<<""
			": Current speed: "<<Vc<<"; average speed: "<<Va<<"; and hello interval: "<<m_helloInterval.GetSeconds());
}
...
RoutingProtocol::Start ()
{
  ...
  m_energyTimer.SetFunction (&RoutingProtocol::EnergyCalculator, this);
  m_energyTimer.Schedule (Seconds (1));
}
          ...
          if (dst == iface.GetBroadcast () || dst.IsBroadcast ())
            {
              ...
              if (lcb.IsNull () == false)
              ...
              else
                {
                  ...
                  dropCount = dropCount + 1;		//my implementation
...
RoutingProtocol::SendRequest (Ipv4Address dst)
{
  ...
  //SetHelloInterval();											//my implementation
  rreqHeader.SetHelloInterval(m_helloInterval.GetSeconds());	//my implementation
  NS_LOG_UNCOND ("\n\n\n\n\n\n\n\n\n\n\nRREQ hello interval set as: "<<rreqHeader.GetHelloInterval());
  ...
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j =
         m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
    {
      ...
      overHead = overHead + packet->GetSize();
...
RoutingProtocol::RecvAodv (Ptr<Socket> socket)
{
  ...
  recOverHead = recOverHead + packet->GetSize();
...
RoutingProtocol::RecvRequest (Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
  ...
  Time recHelloInterval = Seconds(rreqHeader.GetHelloInterval());
  NS_LOG_UNCOND ("\n\n\n\n\n\n\n\n\nReceived RREQ hello interval time: "<<recHelloInterval.GetSeconds());

  rreqHeader.SetHelloInterval(m_helloInterval.GetSeconds());
  ...
  if (!m_routingTable.LookupRoute (src, toNeighbor))
    {
      ...
      RoutingTableEntry newEntry (dev, src, false, rreqHeader.GetOriginSeqno (),
                                  ...
                                  1, src, m_allowedHelloLoss * recHelloInterval);
      m_routingTable.AddRoute (newEntry);
    }
  else
    {
      toNeighbor.SetLifeTime (m_allowedHelloLoss * recHelloInterval);
  ...
  m_nb.Update (src, Time (m_allowedHelloLoss * recHelloInterval));
  ...
  if (gratRep)
    {
      ...
      overHead = overHead + packetToDst->GetSize();
...
RoutingProtocol::ProcessHello (RrepHeader const & rrepHeader, Ipv4Address receiver )
{
  ...
  if (!m_routingTable.LookupRoute (rrepHeader.GetDst (), toNeighbor))
    {
      ...
      RoutingTableEntry newEntry (/*device=*/ dev, /*dst=*/ rrepHeader.GetDst (), /*validSeqNo=*/ true, /*seqno=*/ rrepHeader.GetDstSeqno (),
                                              /*iface=*/ m_ipv4->GetAddress (m_ipv4->GetInterfaceForAddress (receiver), 0),
                                              /*hop=*/ 1, /*nextHop=*/ rrepHeader.GetDst (), /*lifeTime=*/ m_allowedHelloLoss * rrepHeader.GetLifeTime ());
      	  	  //rrepHeader.GetLifeTime () was edited to be multiplied by m_allowedHelloLoss, originally it was only rrepHeader.GetLifeTime ()
      m_routingTable.AddRoute (newEntry);
    }
  else
    {
      //toNeighbor.SetLifeTime (std::max (Time (m_allowedHelloLoss * *m_helloInterval), toNeighbor.GetLifeTime ()));
      toNeighbor.SetLifeTime (std::max (Time (m_allowedHelloLoss * rrepHeader.GetLifeTime () /*m_helloInterval*/), toNeighbor.GetLifeTime ()));
  ...
  if (m_enableHello)
    {
      //default m_nb.Update (rrepHeader.GetDst (), Time (m_allowedHelloLoss * m_helloInterval));

	  /*
	   * proposed and implemented by Imtiaz Mahmud (imtiaz.tee@gmail.com)
	   */
	  if(!m_nb.IsNeighbor(rrepHeader.GetDst()) && networkDensity<2.5)
	  {
		  SendHello();	//proposed protocol
	  }


      m_nb.Update (rrepHeader.GetDst (), Time (m_allowedHelloLoss * rrepHeader.GetLifeTime () /*m_helloInterval*/));
      NS_LOG_UNCOND ("\n\n\n\n\n\n\n\n\n\n\nSend Hello interval received: "<<rrepHeader.GetLifeTime ().GetSeconds());
    }
}
  ...
  if (m_lastBcastTime > Time (Seconds (0)))
    {
      ...
      NS_LOG_UNCOND ("\n\n\n" << m_ipv4->GetObject<Node>()->GetId() << " Hello deferred due to last bcast at:" << m_lastBcastTime.GetSeconds());
    }
  else
    {
      SendHello ();
  ...
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j = m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
    {
     ...
     // RrepHeader helloHeader (/*prefix size=*/ 0, /*hops=*/ 0, /*dst=*/ iface.GetLocal (), /*dst seqno=*/ m_seqNo,
      ...
      NS_LOG_UNCOND ("\n\n\n\n\n\n\n\n\n\n\nSend Hello interval set: "<<m_helloInterval.GetSeconds());
      ...
      sentHelloPackets += 1;
      sentHelloIntervals += m_helloInterval.GetSeconds();
  ...
  if (m_enableHello)
    {
      ...
      NS_LOG_UNCOND (m_ipv4->GetObject<Node>()->GetId() << " Starting at time " << startTime << "ms");
